...
def sendSmsAuto(x):
    ...
    for row in c:
        ...
        SEND_MSG ="""SLT-MOBITEL PEOTV සමඟින් ඔබේ ප්‍රියතම දේශීය හා විදේශීය රූපවාහිනී වැඩසටහන්, ක්‍රීඩා, චිත්‍රපට ඇතුළු තවත් බොහෝ දෑ TRUE HD QUALITY එකෙන් නරඹන්න. දැන්ම ඔබේ PEOTV සම්බන්ධතාවය විශේෂ අවුරුදු දීමනාවක් සමඟින් ලබා ගන්න.
අමතන්න - අසංක  0719121494"""
...
#          """
    ...
    conn = db.DbConnection.dbconnSmsPrg(self="")
    c = conn.cursor()
    sql = 'select MOBILE,SLT_NO,SMSID from SMS_SLT_CUS WHERE STAT=0   AND MOD(DBMS_ROWID.ROWID_ROW_NUMBER(SMS_SLT_CUS.ROWID), 10) = ' + str(x)
    c.execute(sql)
    ...
    for row in c:
        ...
        text= """SLT-MOBITEL PEOTV සමඟින් ඔබේ ප්‍රියතම දේශීය හා විදේශීය රූපවාහිනී වැඩසටහන්, ක්‍රීඩා, චිත්‍රපට ඇතුළු තවත් බොහෝ දෑ TRUE HD QUALITY එකෙන් නරඹන්න. දැන්ම ඔබේ PEOTV සම්බන්ධතාවය විශේෂ අවුරුදු දීමනාවක් සමඟින් ලබා ගන්න.
අමතන්න - අසංක  0719121494"""
        try:
            # create a cursor
...
def sendSmsManual2(x):
    ...
    conn = db.DbConnection.dbconnSmsPrg(self="")
    c = conn.cursor()
    sql = 'select MOBILE,SLT_NO,SMSID from SMS_SLT_CUS2 WHERE STAT=0   AND MOD(DBMS_ROWID.ROWID_ROW_NUMBER(SMS_SLT_CUS2.ROWID), 10) = ' + str(x)
    c.execute(sql)
...
def sendSmsManual3(x):
    ...
    conn = db.DbConnection.dbconnSmsPrg(self="")
    c = conn.cursor()
    sql = 'select MOBILE,SLT_NO,SMSID from SMS_SLT_CUS3 WHERE STAT=0   AND MOD(DBMS_ROWID.ROWID_ROW_NUMBER(SMS_SLT_CUS3.ROWID), 10) = ' + str(x)
    c.execute(sql)
